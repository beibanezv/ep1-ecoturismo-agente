...
TITULO = "Figura 1. Arquitectura del agente replanificador de itinerarios"

# (texto, tipo, texto_desvio, etiqueta_desvio, tipo_desvio)
# tipo: "box" | "decision"; tipo_desvio: "guard" (conflicto) | "ok" (camino alterno)
FILAS = [
    ("Pedido del cliente: actividad + fecha", "box", None, None, None),
    ("Recuperador RAG (ChromaDB): paquete top-1", "box", None, None, None),
    ("Verificacion secuencial: senderos + clima (Open-Meteo) + guia", "box", None, None, None),
    ("Decision: hay conflicto (sendero / clima / guia / temporada)?", "decision",
     "LLM (Groq) + SISTEMA_BASE: itinerario con citas [F#] / [T#]", "no", "ok"),
    ("Replanificador: reconsulta k=8 y evalua temporada + senderos + clima + guia", "box", None, None, None),
    ("Decision: hay alternativa viable?", "decision",
     "Respuesta honesta: sin alternativa inventada", "no", "guard"),
    ("LLM (Groq) + SISTEMA_REPLAN: explica el cambio y cita fuentes", "box", None, None, None),
    ("Itinerario final + logs/trace.jsonl", "box", None, None, None),
]
